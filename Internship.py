...
def app():
    try:
        ...
        st.title('Internship Data')
        st.write("------------------------------------------------------------------")

        # Selection Box for Winter/Summer Internship
        internship_type = st.selectbox('Select Internship Type', ['Winter', 'Summer'])
        ...
        if st.button('Show Table'):
            cursor = collection.find({'internship_type': internship_type}, {'_id': 0}).sort([('rollno', 1)])
